...
export function changeClass(el, newClass, oldClass) {
  if (!el) {
    return
  }
...
export function _setInterval(context, eventName, intervalFunc, frequency) {
  if (!context._interval[eventName]) {
    context._interval[eventName] = setInterval(
...
export function getBuffered2(vbuffered, maxHoleDuration = 0.5) {
  //ref: hls.js
  let buffered = []
  ...
  return new TimeRange(buffered_2)
}
